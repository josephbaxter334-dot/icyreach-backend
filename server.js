...
async function runWarmupPool() {
  if (!sb) return;
  try {
    // Get all accounts opted into warmup
    const { data: accounts } = await sb.from('email_accounts')
    ...
    // Each account sends to 2-3 random other accounts
    for (const sender of accounts) {
      if (!sender.app_password) continue;

      // How many warmup emails to send today based on week
      const days = sender.warmup_start_date
        ? Math.floor((Date.now() - new Date(sender.warmup_start_date)) / 86400000)
        : 0;
      const week = Math.floor(days / 7) + 1;
      const warmupCount = Math.min(week * 2, 6); // 2 per week up to 6/day

      // Pick random recipients (not self)
      ...
      for (const recipient of recipients) {
        try {
          ...
          // Small delay between warmup sends
          await new Promise(r => setTimeout(r, 3000));
        } catch(e) {
          log('WARMUP_SEND_FAIL from=' + sender.email + ' error=' + e.message);
        }
      }
    }
  } catch(e) {
    log('WARMUP_POOL ERROR: ' + e.message);
  }
}
