  ...
  try {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        // client_reference_id is the user's Supabase ID passed via the Stripe link
        ...
        if (userId && subscriptionId) {
          ...
          try {
            ...
            await sb.from('user_plans').upsert({
              id: userId, plan, stripe_customer_id: customerId,
              stripe_subscription_id: subscriptionId,
              plan_started_at: new Date().toISOString(),
              updated_at: new Date().toISOString()
            }, { onConflict: 'id' });
            log('CHECKOUT_COMPLETE userId=' + userId + ' plan=' + plan);
