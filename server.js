    ...
    if (step.type === 'email' || step.type === 'scout_email') {
      ...
      if (step.type === 'scout_email') {
        ...
        const { data: keys } = await sb.from('api_keys')
          .select('key_value').eq('user_id', campaign.user_id)
          .eq('service', 'groq').eq('is_active', true).order('usage_count', { ascending: true });
        const groqKeys = (keys || []).map(k => k.key_value);
        if (groqKeys.length) {
          const generated = await generateScoutEmail(prospect, settings, groqKeys);
        ...
        const { data: template } = await sb.from('templates')
          .select('*').eq('id', step.templateId).single();
        if (!template) {
          log('SEQUENCE skip — template not found step=' + JSON.stringify(step));
          await sb.from('prospects').update({ current_step: stepIndex + 1, next_send_at: new Date().toISOString() }).eq('id', prospect.id);
          return;
        }
      ...
      const unsubLink = BACKEND_URL + '/unsub/' + prospect.id;
      body = body.replace(/\[Unsubscribe\]/g, unsubLink).replace(/\{\{unsubscribe_link\}\}/g, unsubLink);
      ...
      try {
        ...
        await sb.from('prospects').update({
          ...
          next_send_at: isLastStep ? null : new Date().toISOString() // immediately process next step (could be a wait)
        }).eq('id', prospect.id);
...
async function generateScoutEmail(prospect, settings, groqKeys) {
  const biz = {
...
Write a cold email for this prospect. Return ONLY a JSON object: {"subject":"...","body":"..."}

Prospect:
- Name: ${prospect.first_name} ${prospect.last_name}
- Company: ${prospect.company}
- Personalized research: ${prospect.personalized_line || 'None'}
- Research summary: ${prospect.research_data?.summary || 'None'}

Sender's business:
- What they sell: ${biz.product}
- Ideal customer: ${biz.icp}
- Problem they solve: ${biz.problem}
- Value/result: ${biz.value}
- Offer: ${biz.offer}
- Proof: ${biz.proof}

Rules:
- Under 150 words
- Start with the personalized line if available
...
- Never lie or fabricate facts`;
  ...
  for (const key of groqKeys) {
    try {
      const resp = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'llama-3.1-8b-instant', messages: [{ role: 'user', content: prompt }], max_tokens: 500, temperature: 0.7 })
      });
      const data = await resp.json();
      if (data.choices?.[0]?.message?.content) {
        const raw = data.choices[0].message.content.trim();
  ...
  return { subject: 'Quick question', body: 'Hi ' + prospect.first_name + ',\n\n' + (prospect.personalized_line || '') + '\n\nWould you be open to a quick call?\n\nBest' };
}
...
app.get('/track/:prospectId/:step', async (req, res) => {
  ...
  if (sb) {
    try {
      await sb.from('prospects').update({ last_opened_at: new Date().toISOString() }).eq('id', prospectId);
